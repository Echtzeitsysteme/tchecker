...
public:
  ...
  */
  visited_map_t(tchecker::clock_id_t no_of_virtual_clocks, std::shared_ptr<tchecker::vcg::vcg_t> A, std::shared_ptr<tchecker::vcg::vcg_t> B);

  /*!
   \brief Copy Constructor
   \param t : the visited map to copy
  */
  visited_map_t(visited_map_t & t, std::shared_ptr<tchecker::vcg::vcg_t> A, std::shared_ptr<tchecker::vcg::vcg_t> B);
  ...
  */
  ...
  /*!
   ...
   \param second : second symbolic state
  */
  bool check_and_add_pair(tchecker::zg::state_sptr_t first, tchecker::zg::state_sptr_t second);

  /*!
  \brief checks whether the two given symbolic states form a subset of the symbolic states saved in visited map
  \param first : first symbolic state
  \param second : second symbolic state
  \note only returns true if visited map contains superset but might return false incorrectly. Always returns true if the given
  pair of symbolic states form a subset of a single pair of symbolic states in visited map
  */
...
private:
  ...
  */
  ...
  /*!
   \brief returns an iterator, starting after the end, iterating over pairs of one visited_map_key_t and one zone container ptr
   each
  */
