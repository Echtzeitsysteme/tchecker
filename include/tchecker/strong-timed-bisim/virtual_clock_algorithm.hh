...
private:
  ...
  struct custom_hash {
    size_t operator()(const std::pair<tchecker::vloc_sptr_t, tchecker::vloc_sptr_t> &to_hash) const {
      size_t h = TCHECKER_STRONG_TIMED_BISIM_VIRTUAL_CLOCK_ALGORITHM_HH_SEED;
      for (size_t i = 0; i < to_hash.first->capacity(); i++){
        boost::hash_combine(h, (*to_hash.first)[i]);
      }
      for (size_t i = 0; i < to_hash.second->capacity(); i++){
        boost::hash_combine(h, (*to_hash.second)[i]);
      }
      return h;
    }
  };
  ...
  struct custom_equal {
    bool operator() (const std::pair<tchecker::vloc_sptr_t, tchecker::vloc_sptr_t> &p1, const std::pair<tchecker::vloc_sptr_t, tchecker::vloc_sptr_t> &p2) const {
      return (*(p1.first) == *(p2.first)) && (*(p1.second) == *(p2.second));
    }
  };
  
  /*!
   \brief Unordered map mapping a pair of vlocs to a pair of zone containers
  */
  using visited_map_t = std::unordered_map<std::pair<tchecker::vloc_sptr_t, tchecker::vloc_sptr_t>, std::pair<std::shared_ptr<tchecker::zone_container_t<zg::zone_t>>, std::shared_ptr<tchecker::zone_container_t<zg::zone_t>>>, custom_hash, custom_equal>;
