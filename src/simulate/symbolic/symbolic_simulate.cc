                      ...
                      std::size_t nsteps)
{
  ...
  if (k == NO_SELECTION)
    return state_space;
  graph_t::node_sptr_t previous_node = g.add_node(zg->state(v[k]));
  ...
  for (std::size_t i = 0; i < nsteps; ++i) {
    ...
    graph_t::node_sptr_t node = g.add_node(zg->state(v[k]));
...
/*!
 ...
 NO_SELECTION otherwise
*/
static std::size_t interactive_select(display_t & display, tchecker::zg::const_state_sptr_t const & s,
                       ...
                       std::map<std::string, std::string> const & starting_state_attributes)
{
  ...
  graph_t::node_sptr_t previous_node{nullptr};
  std::size_t k = NO_SELECTION;

  if (starting_state_attributes.empty()) {
    // start simulation from initial states (interactive selection)
    zg->initial(v);
    k = interactive_select(*display, tchecker::zg::const_state_sptr_t{nullptr}, v);
  }
  else {
    // start simulation from specified state
    zg->build(starting_state_attributes, v);
    assert(v.size() <= 1);
    k = (v.size() == 0 ? NO_SELECTION : 0); // select state if any
  }
  ...
  do {
    ...
    std::size_t k = interactive_select(*display, previous_node->state_ptr(), v);
    if (k == NO_SELECTION)
      break;
