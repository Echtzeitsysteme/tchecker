    ...
    bool generate_witness)
{
  ...
  for(size_t i = 0; i < 2; ++i) {
    ...
    std::shared_ptr<tchecker::vcg::vcg_t> vcg{tchecker::vcg::factory(extended_system, 0 == i, systems[0], systems[1], urgent_or_committed_clock, tchecker::ts::SHARING, tchecker::zg::DISTINGUISHED_SEMANTICS, tchecker::zg::EXTRA_M_LOCAL, block_size, table_size)};
    vcgs.push_back(vcg);
  }
