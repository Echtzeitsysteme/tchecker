...
bool cont_dag_t::create_cont_from_non_bisim_cache(tchecker::strong_timed_bisim::non_bisim_cache_t &non_bisim_cache, node_t &src)
{
  ...
  while (!to_process.empty()) {
    ...
    if (cur->is_leaf(_init_states.first, _init_states.second, _vcg1, _vcg2)) {
      ...
      bool gate = std::any_of(cur->valuation().first->begin(), cur->valuation().first->end(),
                              [max_delay](const clock_rational_value_t & val) { return val < max_delay; });

      gate |= std::any_of(cur->valuation().second->begin(), cur->valuation().second->end(),
                          [max_delay](const clock_rational_value_t & val) { return val < max_delay; });

      auto delay = cur->max_delay(non_bisim_cache.entry(cur->location_pair()), _max_delay, _vcg1, _vcg2);
...
bool cont_dag_t::add_non_bisim_action_transition(tchecker::strong_timed_bisim::non_bisim_cache_t &non_bisim_cache, node_t &src)
{
  assert(!src.is_leaf(_init_states.first, _init_states.second, _vcg1, _vcg2));
  ...
  for (auto & symbol : *avail_events) {
    ...
    for (size_t idx_1 = 0; idx_1 < trans_1->size(); idx_1++) {
      ...
      for (size_t idx_2 = 0; idx_2 < trans_2->size(); idx_2++) {
        ...
        if(non_bisim_cache.is_cached(cur_init_node->location_pair(), cur_init_node->valuation().first)) {
    ...
    for (size_t idx_1 = 0; idx_1 < trans_1->size(); idx_1++) {
      if(std::all_of(return_values[idx_1].begin(), return_values[idx_1].end(), [](bool v){ return v; })) {
        for(std::size_t idx_2 = 0; idx_2 < trans_2->size(); idx_2++) {
          add_nodes_and_edges_of(*sub_dag[idx_1][idx_2]);
        }
        return true;
      }
    }
...
void cont_dag_t::add_nodes_and_edges_of(cont_dag_t & other)
{
  for(auto cur : *other._nodes) {
